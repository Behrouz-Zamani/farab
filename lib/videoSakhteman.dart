...
import 'models/videoCivilModels.dart';

class Sakhtemanvideo extends StatefulWidget {
  @override
  State<Sakhtemanvideo> createState() => _Sakhtemanvideo();
}

class _Sakhtemanvideo extends State<Sakhtemanvideo> {
  int selected = 0;
  bool isPlaying = true;
  ...
  void _playVideo({int index = 0, bool init = false}) {
    if (index < 0 || index >= videocivilmodel.length) return;
    if (!init) {
      _controller.pause();
    }
    setState(() {
    ...
    _controller = VideoPlayerController.network(videocivilmodel[_currentIndex].url)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('تلویزیون فراب - حوزه ساختمان',style: TextStyle(fontSize: 14),),
            ...
            Container(
             // color: const Color.fromARGB(255, 255, 255, 255),
              ...
              child: ListView.builder(
                itemCount: videocivilmodel.length,
